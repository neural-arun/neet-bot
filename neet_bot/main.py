...
class WebServerHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if path in ['/', '/index.html']:
            ...
            self._json_response({'chapters': chapters, 'total': len(chapters), 'subject': subject})
            ...
            if not chapters_list or chapters_list == ['']:
                ...
                q_list = qs.get_random_questions(chapters_list[0], count, subject)
            else:
                q_list = qs.get_random_questions_multi(chapters_list, count, subject)

            self._json_response({'questions': q_list, 'count': len(q_list)})
        elif path == '/api/full_mock':
            q_list = qs.get_full_neet_mock()
            self._json_response({'questions': q_list, 'count': len(q_list)})
    ...
    def do_POST(self):
        ...
        if parsed.path == '/api/analyze':
            ...
            try:
                ...
                ch = req_data.get('chapter', 'NEET Practice')
...
def run_bot_polling(token, bot_name, subject='biology'):
    if not token:
        logging.warning(f"[{bot_name}] Token not configured. Polling skipped.")
        return
    ...
    while True:
        try:
            logging.info(f"[{bot_name}] Starting Telegram bot polling for {subject}...")
            app = build_app(token, subject=subject)
            app.run_polling(allowed_updates=['message', 'callback_query'], stop_signals=None, close_loop=False)
        except Exception as e:
            logging.error(f"[{bot_name}] Polling error for {bot_name}: {e}. Retrying in 15 seconds...")
            time.sleep(15)
...
def main():
    ...
    init_db()
    logging.info("Starting Master NEET Multi-Bot Engine...")

    # Start 3 Telegram Bot Polling Threads
    bot_threads = [
        threading.Thread(target=run_bot_polling, args=(config.BIOLOGY_BOT_TOKEN, "GPT Biology Bot", "biology"), daemon=True),
        threading.Thread(target=run_bot_polling, args=(config.PHYSICS_BOT_TOKEN, "GPT Physics Bot", "physics"), daemon=True),
        threading.Thread(target=run_bot_polling, args=(config.CHEMISTRY_BOT_TOKEN, "GPT Chemistry Bot", "chemistry"), daemon=True)
    ]
